...
def run_benchmarking(logger: PopcornOutput, pool: multiprocessing.Pool, tests: list[TestCase]):
    ...
    for idx, test in enumerate(tests):
        ...
        if isinstance(result, Stats):
            # convert mean to microseconds
            mean_us = result.mean / 1_000
            mean_us_list.append(mean_us)
    ...
    # print collected means and geometric mean
    if mean_us_list:
        geo_mean = math.exp(sum(math.log(x) for x in mean_us_list) / len(mean_us_list))
        logger.print(f"Collected mean times (µs): {mean_us_list}")
        logger.print(f"Geometric mean (µs): {geo_mean}")
