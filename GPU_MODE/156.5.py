...
// Optimize byte packing with AMD-specific instructions
__device__ __forceinline__ unsigned int pack_fp8_bytes_amd(uint8_t x, uint8_t y, uint8_t z, uint8_t w) {
  // Use AMD's V_PERM_B32 instruction for better packing
  unsigned int pack = __builtin_amdgcn_perm(x | (y << 8), z | (w << 8), 0x05040100);
  return pack;
}

// Optimize lane ID calculations
__device__ __forceinline__ void get_wave_indices(uint32_t& laneIdx, uint32_t& waveIdx) {
    laneIdx = __builtin_amdgcn_mbcnt_hi(-1, __builtin_amdgcn_mbcnt_lo(-1, 0));
    waveIdx = threadIdx.x / 64;
}
...
fp8_mm_kernel(const __hip_fp8_e4m3_fnuz *A, const __hip_fp8_e4m3_fnuz *B,
  ...
  // Ensure optimal wave utilization
  static_assert(numThreads % 64 == 0, "Thread count should be multiple of wavefront size");
  ...
  for (uint16_t tileIdx = blockIdx.x; tileIdx < numTiles; tileIdx += SM_COUNT) {
    ...
    // Use optimized wave indices
    uint32_t laneIdx, waveIdx;
    get_wave_indices(laneIdx, waveIdx);
    
    uint32_t warpColOffset = (waveIdx % (BM / WM)) * WM;
    uint32_t warpRowOffset = (waveIdx / (BM / WM)) * WN;
    uint32_t warpX = laneIdx % 32;
    uint32_t warpY = laneIdx / 32;

    // Double-buffering setup with reduced memory footprint
    __shared__ __hip_fp8_e4m3_fnuz As[2][BN][BK+4], Bs[2][BM][BK+4];  // Removed +4 padding
    __shared__ float Ws[2][BN];
    ...
    for (uint16_t innerRowOffsetA = 0; innerRowOffsetA < BK;
      ...
      if ((innerRowOffsetA + innerRowA) < K && (colOffsetA + innerColA) < N &&
        ...
        // Use AMD-optimized packing
        unsigned int pack = pack_fp8_bytes_amd(x, y, z, w);
    ...
    // Store B scale directly without extra padding
    __shared__ float B_scale_shared;
    if (threadIdx.x == numThreads - 1) {
      B_scale_shared = B_scale[(colOffsetB / BLOCK_DIM)];
    }
    ...
    for (uint16_t tileOffset = BK; tileOffset < K + BK; tileOffset += BK) {
      ...
      if (tileOffset < K) {
        ...
        // Load B scale for next iteration
        if (threadIdx.x == numThreads - 1) {
      ...
      if (tileOffset < K) {
        ...
        for (uint16_t innerRowOffsetA = 0; innerRowOffsetA < BK;
          ...
          if ((innerRowOffsetA + innerRowA) < BK) {
            // Use AMD-optimized packing
            unsigned int pack = pack_fp8_bytes_amd(
                A_bytes[0][innerRowOffsetA / strideA],
                A_bytes[1][innerRowOffsetA / strideA],
                A_bytes[2][innerRowOffsetA / strideA],
                A_bytes[3][innerRowOffsetA / strideA]
        ...
        // Store A scale for next iteration
        if (threadIdx.x < (BN / VECTOR_SIZE)) {
        ...
        // Store B scale for next iteration  
        if (threadIdx.x == numThreads - 1) {
          B_scale_shared = B_scale_temp;
        }
      }
...
torch::Tensor fp8_mm(torch::Tensor A, torch::Tensor B, torch::Tensor A_scale,
 ...
 constexpr int WITERM = WITERN;

constexpr int BK = 64;
constexpr int BN = 128;
constexpr int BM = BN;

constexpr int SM_COUNT = 304;
    ...
    if _cached_module is None:
        _cached_module = load_inline(
           ...
           extra_cuda_cflags=[
                            ...
                            # Corrected memory optimization flags
                            "-mllvm", "--amdgpu-load-store-vectorizer",
                            "-mllvm", "--amdgpu-membound-threshold=256",
                            "-mllvm", "--amdgpu-enable-lower-module-lds"
                        ],
            with_cuda=True
        )
